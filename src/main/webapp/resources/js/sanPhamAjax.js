$(document).ready(function() {
	...
	function ajaxGet(page){
		...
		$.ajax({
			...
			url: "http://localhost:8080/beauteshop/api/san-pham/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, sanPham){
					var sanPhamRow = '<tr>' +
					                  '<td>' + '<img src="/beauteshop/img/'+sanPham.id+'.png" class="img-responsive" style="height: 50px; width: 50px" />'+'</td>' +
					...
					sanPhamRow += '<button class="btn btn-primary btnCapNhatOther" >Cập nhật</button>';
	...
	$(' .otherModal').on('hidden.bs.modal', function(e) {
		e.preventDefault();
		$("#idDanhMucKhac").val("");
		$("#idSanPhamKhac").val("");				    
    ...
    function ajaxPostOther() {
    	// PREPATEE DATA
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/beauteshop/api/san-pham/save",
    ...
    $(document).on("click",".btnCapNhatOther", function(event){
		...
		var href = "http://localhost:8080/beauteshop/api/san-pham/"+sanPhamId;
    ...
    $(document).on("click",".btnXoaSanPham", function() {
		...
		if(confirmation){
		  $.ajax({
			  async:false,
			  type : "DELETE",
			  url : "http://localhost:8080/beauteshop/api/san-pham/delete/"+sanPhamId,
    ...
    $(document).on("click",".btnChiTiet", function() {
    	...
    	var href = "http://localhost:8080/beauteshop/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			$('.hinhAnh').attr("src", "/beauteshop/img/"+sanPham.id+".png");
