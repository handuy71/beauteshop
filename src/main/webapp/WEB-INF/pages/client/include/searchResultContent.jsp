		    ...
		    <div class="content-grids">
		    		...
		    		<c:if test = "${loop.index != list.size()-1}">
         				<c:if test = "${loop.index%4 == 0}">
         					...
         					<div class="grid_1_of_4 images_1_of_4 products-info"><a href="sp?id=${sanpham.id}">
									<img style="width: 300px; height: 238px" src="/beauteshop/img/${sanpham.id}.png">
		    	...
		    	<c:if test = "${list.size() != 0}">
            		...
            		<c:if test = "${currentPage != 1}">
                		<a href="/beauteshop/search?name=${name}&page=${currentPage-1}&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">Back</a>
                	</c:if>
                	<c:if test = "${currentPage == 1}">	
        				<a class="current">1</a>
        			</c:if>
        			<c:if test = "${currentPage != 1}">	
        				<a href="/beauteshop/search?name=${name}&page=1&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">1</a>
        			</c:if>
        			...
        			<c:forEach var="pag" items="${pageList}" varStatus="loop">
        				...
        				<c:if test = "${currentPage != pag}">	
        					<a href="/beauteshop/search?name=${name}&page=${pag}&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">${pag}</a>
        				</c:if>
      				</c:forEach>

            		<c:if test = "${currentPage != totalPage}">
                		<a href="/beauteshop/search?name=${name}&page=${currentPage+1}&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">Next</a>	
                	</c:if>
		    		...
		    		<h4>Lọc sản phẩm</h4>		    			
        				...
        				<div class="select-range">
        					<select name="range" class="form-control">
								...
								<option value="duoi-100.000d">Dưới 100.000đ</option>
								<option value="100.000d-den-300.000d">100.000đ đến 300.000đ</option>
								<option value="300.000d-den-500.000d">300.000dđ đến 500.000đ</option>
								<option value="500.000d-den-1.000.000d">500.000đ đến 1.000.000đ</option>
								<option value="tren-1.000.000d">Trên 1.000.000đ</option>					
        					</select>
        				</div>   				
        				<p>Danh mục</p>
        				<div class="select-range">
