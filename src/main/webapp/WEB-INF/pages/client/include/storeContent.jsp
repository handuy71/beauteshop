		    ...
		    <div class="content-grids">
		    		...
		    		<c:if test = "${loop.index != list.size()-1}">
         				<c:if test = "${loop.index%4 == 0}">
         					...
         					<div class="grid_1_of_4 images_1_of_4 products-info" style=" width: 232px; height: 400px;"><a href="sp?id=${sanpham.id}">
									<img style="width: 225px; height: 230px" src="/beauteshop/img/${sanpham.id}.png">
		    	...
		    	<c:if test = "${list.size() != 0}">
            		...
            		<c:if test = "${currentPage != 1}">
                		<a href="/beauteshop/store?page=${currentPage-1}&range=${range}&brand=${brand}&manufactor=${manufactor}">Back</a>
                	</c:if>
                	<c:if test = "${currentPage == 1}">	
        				<a class="current">1</a>
        			</c:if>
        			<c:if test = "${currentPage != 1}">	
        				<a href="/beauteshop/store?page=1&range=${range}&brand=${brand}&manufactor=${manufactor}">1</a>
        			</c:if>
        			...
        			<c:forEach var="pag" items="${pageList}" varStatus="loop">
        				...
        				<c:if test = "${currentPage != pag}">	
        					<a href="/beauteshop/store?page=${pag}&range=${range}&brand=${brand}&manufactor=${manufactor}">${pag}</a>
        				</c:if>
      				</c:forEach>

            		<c:if test = "${currentPage != totalPage}">
                		<a href="/beauteshop/store?page=${currentPage+1}&range=${range}&brand=${brand}&manufactor=${manufactor}">Next</a>	
                	</c:if>
				</div>
		    	</c:if>
